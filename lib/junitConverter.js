...
class JunitConverter {
    ...
    async convert(options, json) {
        ...
        const stats = mochawesomeCommon_1.MochawesomeCommon.createStats(this.suites.length, tests, suiteFailures, options.skippedAsPending ? pending : 0, !options.skippedAsPending ? pending : 0, options, pendingPercent, 0, Math.ceil(duration * 1000));
        const mochawesome = {
            stats: stats,
            results: results,
        };
